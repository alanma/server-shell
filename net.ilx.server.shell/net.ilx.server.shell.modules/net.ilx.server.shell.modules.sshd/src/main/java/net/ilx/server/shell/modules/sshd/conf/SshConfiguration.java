package net.ilx.server.shell.modules.sshd.conf;
...
import net.ilx.server.shell.core.server.spring.components.ApplicationEventType;
import net.ilx.server.shell.core.server.spring.components.CustomApplicationEvent;
import net.ilx.server.shell.core.server.spring.components.commands.CommandSupportConfiguration;
import net.ilx.server.shell.core.server.spring.components.commands.EnableCommands;
import net.ilx.server.shell.modules.sshd.SpringCommandProcessor;
import net.ilx.server.shell.modules.sshd.SshCommandDispatcher;
import net.ilx.server.shell.modules.sshd.shell.command.CommandProcessor;
import net.ilx.server.shell.modules.sshd.shell.console.ssh.SshShellFactory;
import net.ilx.server.shell.modules.sshd.shell.runtime.threadio.ThreadIOImpl;
import net.ilx.server.shell.modules.sshd.shell.service.threadio.ThreadIO;
...
@Configuration
@EnableCommands
public class SshConfiguration implements
    ...
    public KeyPairProvider keyPairProvider() {
        ...
        try {
            ...
            keyProvider = new FileKeyPairProvider(
                    new String[] { file.getAbsolutePath() });
        } catch (FileNotFoundException e) {
            throw new IllegalStateException(
                    "sshd hostkey MUST be defined in classpath");
        }
    ...
    public SshServer sshServer() {
        // ilx: see
        // https://github.com/eclipse/rt.equinox.bundles/blob/master/bundles/org.eclipse.equinox.console.ssh/src/org/eclipse/equinox/console/ssh/SshServ.java
    ...
    public CommandProcessor springCommandProcessor() {
        ...
        SimpleParser simpleCommandParser = commandSupportConfiguration
                .simpleCommandParser();
        SimpleExecutionStrategy executionStrategy = commandSupportConfiguration
                .simpleExecutionStrategy();
        CommandProcessor commandProcessor = new SpringCommandProcessor(tio,
                simpleCommandParser, executionStrategy);
    ...
    public void onApplicationEvent(CustomApplicationEvent event) {
        ApplicationEventType type = event.getType();
        switch (type) {
        case START:
            startSshServer();
            break;
    ...
    private void stopSshServer() {
        try {
            LOG.info("Stoping SSH server");
            sshServer().stop();
            LOG.info("SSH server stopped.");
        } catch (InterruptedException e) {
            String msg = "Unable to stop SSH server.";
            LOG.error(msg, e);
    ...
    private void startSshServer() {
        try {
            ...
            String msg = "Unable to start SSH server.";
            LOG.error(msg, e);
            throw new IllegalStateException(msg, e);
        }
    }
