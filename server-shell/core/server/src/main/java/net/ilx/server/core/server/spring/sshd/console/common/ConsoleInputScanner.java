...
import net.ilx.server.core.server.spring.sshd.console.completion.CompletionHandler;
...
public class ConsoleInputScanner extends Scanner {
    ...
    private void sendQuitToConsole() throws IOException {
    	// net.ilx.server.core.server.spring.sshd.console.ssh.Console.getLine(String)
    	newChar(4);
    	newChar(CR);
    	processData();
	}
    ...
    protected void processTab() throws IOException {
    	CompletionHandler completionHandler = new CompletionHandler(session);
        ...
        if (completionCandidates.size() == 0) {
        	// ILX : instead of error do nothing....
...
//        buffer.add(CR);
    ...
    private void clear_screen() {
    	// TODO ilx: clear screen?
    	System.out.println("should clear screen!");
	}
